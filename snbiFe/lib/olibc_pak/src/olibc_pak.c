...
olibc_pak_recv (olibc_pak_hdl pak_hdl, int fd, uint32_t offset_bytes)
{
    ...
    if (pak_hdl->addr_family == AF_INET) {
        ...
        rmsghdr.msg_name = (caddr_t) &pkt->src_sock_addr;
    ...
    for (rcmsgp = CMSG_FIRSTHDR(&rmsghdr);
        ...
        if (rcmsgp->cmsg_level == IPPROTO_IP && 
            ...
            if (dst_in4_pkt_info->ipi_ifindex) {
                pkt_hdl->in_ifindex = dst_in4_pkt_info->ipi_ifindex;
                pak_hdl->data_set_flags |= OLIBC_IN_IFHNDL_SET;
            }
        }

        if (rcmsgp->cmsg_level == IPPROTO_IPV6 && 
                rcmsg->cmsg_type == IPV6_PKTINFO) {
            struct in6_pktinfo *dst_in6_pkt_info = NULL;
            struct sockaddr_in6 *s = NULL;

            dst_in6_pkt_info = CMSG_DATA(rcmsgp);
            ...
            if (dst_in6_pkt_info->ipi6_ifindex) {
                pkt_hdl->in_ifindex = dst_in6_pkt_info->ipi6_ifindex;
                pak_hdl->data_set_flags |= OLIBC_IN_IFHNDL_SET;
            }
...
olibc_pak_send (olibc_pak_hdl pak_hdl,  int fd, uint32_t offset_bytes)
{
    ...
    if (pak_hdl->data_set_flags & OLIBC_DEST_SOCKADDR_SET) {
    ...
    if (pak_hdl->data_set_flags & 
        ...
        if (pak_hdl->addr_family == AF_INET) {
            ...
            dst_in6_pkt_info = CMSG_DATA(scmsgp);
            ...
            if (pak_hdl->data_set_flags & OLIBC_SRC_SOCKADDR_SET) { 
                ...
                memset(&dst_in6_pkt_info->ipi6_addr, &s6->sin6_addr,
...
olibc_retval_t
olibc_pak_get_src_address (olibc_pak_hdl pak_hdl,
...
olibc_retval_t
olibc_pak_set_dst_address (olibc_pak_hdl pak_hdl,
                           struct sockaddr *dst_sock)
{
    if (!pak_hdl) {
        olibc_log_error("\nInvalid input"); 
        return OLIBC_RETVAL_INVALID_INPUT;
    ...
    if (dst_sock->sa_family == AF_INET) {
        memcpy(&pak_hdl->dst_sock_addr, dst_sock, sizeof(struct sockaddr_in));
    }
    if (dst_sock->sa_family == AF_INET6) {
        memcpy(&pak_hdl->dst_sock_addr, dst_sock, sizeof(struct sockaddr_in6));
    }
    pak_hdl->data_set_flags |= OLIBC_DEST_SOCKADDR_SET;
