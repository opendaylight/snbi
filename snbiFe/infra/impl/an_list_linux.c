...
{
    return (an_map_olibc_retval(olibc_list_new(list, list_name)));
}

an_cerrno 
an_list_destroy (an_list_t **list)
{
    return(an_map_olibc_retval(olibc_list_destroy(list)));
}
                          ...
                          void* data, an_list_comp_handler comp_handler)
{
    ...
    if (CERR_IS_OK(an_map_olibc_retval(
                   olibc_list_lookup_node(list, elem, 
                                           data, comp_handler, 
                                           &return_data)))) {
        return return_data;
    }
...
{
    olibc_list_enqueue_node(list, data);
}
