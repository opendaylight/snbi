...
extern boolean an_acp_cnp_uninit(void);
...
an_event_interface_up (an_if_t ifhndl)
{
    ...
    if (domain_cert.len ) {
        an_cd_start_probing_on_interface(ifhndl);
    } else {
        an_cd_start_on_interface(ifhndl);
    }
}
...
an_event_interface_down (an_if_t ifhndl)
{
    ...
    an_cd_stop_on_interface(ifhndl);
}
...
#ifndef AN_IDP_PUSH
    an_idp_send_intent_version_message_v2(nbr->udi);
#else
    an_idp_send_intent_message(nbr->udi);
#endif
...
an_event_acp_negotiate_security_with_nbr_link (an_nbr_t *nbr, 
                                              an_nbr_link_spec_t *nbr_link_data)
{
    an_acp_negotiate_security_type_per_nbr_link(nbr, nbr_link_data);
}
...
an_event_generic_timer_expired (void)
{
    ...
    if (an_anra_is_live() && an_acp_is_initialized()) {
        ...
        DEBUG_AN_LOG(AN_LOG_SRVC_EVENT, AN_DEBUG_INFO, NULL,
               "\n%s Generic timer expired. Adding ANR address [%s] to mdns cache",
